        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:flex lg:gap-8 gap-6 items-stretch">
          {promos.map((pkg, i) => {
            ...
            return (
              ...
              >
                <div className={`relative rounded-3xl overflow-hidden shadow-2xl transition-all duration-700 ease-in-out group ${isHovered && window.innerWidth >= 1024 ? 'shadow-[0_20px_80px_rgba(28,119,183,0.4)]' : ''
                  }`}
                  style={{
                  ...
                  {/* Overlay gradient */}
                  <div className={`absolute inset-0 transition-all duration-700 ease-in-out ${isHovered && window.innerWidth >= 1024
                  ...
                  {/* Contenido */}
                  <div className={`relative z-10 h-full p-6 md:p-8 lg:p-10 text-white transition-all duration-700 ease-in-out ${isHovered && window.innerWidth >= 1024
                      ? 'flex flex-col lg:flex-row lg:items-center lg:justify-between'
                      : 'flex flex-col justify-end'
                    }`}>
                    <div className={`transition-all duration-700 ease-in-out ${isHovered && window.innerWidth >= 1024 ? 'lg:max-w-2xl' : 'w-full'
                      }`}>
                      {/* Badge */}
                      {badge && (
                      ...
                      {/* Título */}
                      <h3 className={`font-bold mb-3 transition-all duration-700 ease-in-out ${isHovered && window.innerWidth >= 1024 ? 'text-4xl lg:text-5xl' : 'text-2xl md:text-3xl'
                        }`}>
                        {pkg.nombre}
                      </h3>

                      {/* Descripción corta */}
                      <p className={`text-base md:text-lg mb-4 transition-all duration-700 ease-in-out ${isHovered && window.innerWidth >= 1024 ? 'lg:text-xl' : ''
                        }`}
                        style={{
                          display: '-webkit-box',
                      ...
                      {/* Botón CTA */}
                      <button className={`rounded-full px-6 md:px-8 py-2 md:py-3 text-sm md:text-base font-bold border-2 transition-all duration-300 ${isHovered && window.innerWidth >= 1024
