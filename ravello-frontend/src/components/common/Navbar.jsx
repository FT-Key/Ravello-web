...
const Navbar = () => {
  ...
  // Cambia inmediatamente al hacer scroll
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 0); // se activa con el primer pixel de scroll
    };
  ...
  return (
    <nav className="sticky top-0 left-0 right-0 z-50 min-h-[115px]">
      <div
        className={`transition-all duration-500 ${
          isScrolled
            ? "bg-white shadow-lg"
            : "bg-black/90 backdrop-blur-md border-b border-black/60"
        }`}
      >
        {/* Barra superior */}
